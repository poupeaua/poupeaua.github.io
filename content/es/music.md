---
Title: Música
type: "music"

carousel:
    carousel1:
        info: Aprende a tocar la guitarra hoy mismo
        subinfo:  Materiales para estudiantes que aprenden guitarra
    carousel2:
        info: Composiciones
        subinfo: Consulta las músicas electrónicas que he hecho bajo mi nombre artístico, Chill Wind.
    carousel3:
        info: Más información
        subinfo: Descubre la relación entre la música y yo.

courses:
    title: Cursos de guitarra
    intro: "Aquí encontrarás material didáctico que te ayudará a mejorar tanto tus habilidades con la guitarra como tus conocimientos de teoría musical. Los cursos están divididos en 4 categorías. Recomiendo seguir el camino: **Principiante - Intermedio - Avanzado** y completar los cursos en orden. De hecho, cada curso es un ladrillo que te ayuda a entender conceptos más avanzados. La parte **Extra** está pensada para mostrar contenido adicional para una mayor exploración."
    disclaimer: "Nota: este material está destinado a proporcionar herramientas al estudiante como complemento de los cursos con un profesor de guitarra."
    levels:
        beginner: Principiante
        intermediate: Intermedio
        advanced: Avanzado
        extra: Extra

compositions:
    title: Composiciones
    citation: "*\"Escuchas la música, la música te dice lo que tienes que hacer.\"* - Guthrie Govan"

about:
    title: Más información
    experience:
        title: Experiencia musical
        content1: Llevo más de 15 años tocando la guitarra, he tenido la oportunidad de tocar en diferentes grupos, organizar conciertos y hacer jam.
        content2: Me encanta aportar de mi paciencia y motivación a la enseñanza. Enseño la guitarra como me hubiera gustado que me enseñaran a mí cuando empecé. Me centro tanto en la práctica como en la teoría musical.
    mao:
        title: Música electrónica
        content1: En 2017, empecé a crear mi propia música electrónica en el ordenador. ¡Fue muy divertido! Tuve la oportunidad de aprender mucho a lo largo de esta aventura sobre MAO como FlStudio o Ableton.
        content2: De manera teórica, también fue genial aprender sobre nuevos conceptos como la masterización, la mezcla de audio y la composición.

---